...
- **Price pass (1H, every `CHECK_INTERVAL`):** Runs only for 1H. Rules: **monthly pivot** and **EMA 200**. Fetches latest 1H candle + indicators from DB and current OHLC from Binance, runs `run_price_rules()`, applies cooldown, sends one consolidated message per ticker if any alert fires.
...
3. **EMA200** – Current price touches or crosses EMA 200. **Price pass** for 4h, 1d, 1w, 1M (no 1h). Uses fixed EMA from last closed candle.
